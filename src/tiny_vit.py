...
# Adapted from LeViT, Swin Transformer, TinyVIT
...
class Conv2d_LN(torch.nn.Sequential):
    def __init__(self, a, b, ks=1, stride=1, pad=0, dilation=1,
        ...
        bn = torch.nn.LayerNorm(b)
        # bn = torch.nn.LayerNorm([b, 1, 1]) # Normalize over channel, height, width
    ...
    def fuse(self):
        ...
        if c.bias is not None:
            b = bn.bias + (c.bias - bn.weight * bn.weight.mean(dim=[1, 2, 3], keepdim=True))
        else:
            b = bn.bias - bn.weight * bn.weight.mean(dim=[1, 2, 3], keepdim=True)
...
class PatchEmbed(nn.Module):
    def __init__(self, in_chans, embed_dim, resolution, activation):
        ...
        self.seq = nn.Sequential(
            Conv2d_LN(in_chans, n // 2, 3, 2, 1),
            activation(),
            Conv2d_LN(n // 2, n, 3, 2, 1),
        )
...
class MBConv(nn.Module):
    def __init__(self, in_chans, out_chans, expand_ratio,
        ...
        self.conv1 = Conv2d_LN(in_chans, self.hidden_chans, ks=1)
        self.act1 = activation()

        self.conv2 = Conv2d_LN(self.hidden_chans, self.hidden_chans,
                               ks=3, stride=1, pad=1, groups=self.hidden_chans)
        self.act2 = activation()

        self.conv3 = Conv2d_LN(
...
class PatchMerging(nn.Module):
    def __init__(self, input_resolution, dim, out_dim, activation):
        ...
        self.act = activation()
        self.conv1 = Conv2d_LN(dim, out_dim, 1, 1, 0)
        self.conv2 = Conv2d_LN(out_dim, out_dim, 3, 2, 1, groups=out_dim)
        self.conv3 = Conv2d_LN(out_dim, out_dim, 1, 1, 0)
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        attn = (
            ...
            (self.attention_biases[:, self.attention_bias_idxs]
             if self.training else self.ab.to(q.device))
        )
...
class TinyViTBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7,
        ...
        self.local_conv = Conv2d_LN(
            dim, dim, ks=local_conv_size, stride=1, pad=pad, groups=dim)

    def forward(self, x):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, f'{L}, {H}, {W}'
        res_x = x
...
class TinyViT(nn.Module):
    ...
    def forward(self, x):
        # assert False, f'input shape = {x.shape}'
