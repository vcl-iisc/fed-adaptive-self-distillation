...
class DatasetObject:
    ...
    def set_data(self):
        ...
        if not os.path.exists('%s/%s' %(self.data_path, self.name)):
            # Get Raw data
            if self.dataset == 'IN100':
                self.channels = 3; self.height = 224; self.width = 224; self.n_cls = 100;
                print('created dataloaders', f'num_classes = {self.n_cls}')
            
            if self.dataset == 'mnist':
                transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
                trnset = datasets.MNIST(root='%s/Raw' %self.data_path,
            ...
            if self.dataset == 'CIFAR10':
                ...
                trnset = datasets.CIFAR10(root='%s/Raw' %self.data_path,
                                                      train=True , download=True, transform=transform)
            ...
            if self.dataset == 'CIFAR100':
                ...
                trnset = datasets.CIFAR100(root='%s/Raw' %self.data_path,
                                                      train=True , download=True, transform=transform)
            ...
            #     clnt_x, clnt_y = partition_data(trn_x, trn_y, self.n_cls, self.n_client, self.rule_arg, clnt_data_list)
